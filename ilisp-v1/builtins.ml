...
let eval_add arg1 arg2 =
  match arg1 with
  |Cons _ -> raise (Error "add only works on int")
  |Atom astr -> let a = int_of_string astr in
                match arg2 with
                |Cons _ -> raise (Error "add only works with int")
                |Atom bstr -> let b = int_of_string bstr in
                              Atom (string_of_int (a + b))


(* interpret (add_list arg) *)
let rec eval_add_list arg =
  match arg with
  |Atom "nil" -> Atom "0"
  |Atom a -> Atom a
  |Cons (car, cdr) -> eval_add car (eval_add_list cdr)
...
let eval_geq arg1 arg2 =
  ...
  |Atom astr -> let a = int_of_string astr in
                ...
                |Cons _ -> raise (Error "add only works with int")
                |Atom bstr -> let b = int_of_string bstr in
                              if a < b then (Atom "nil") else (Atom "t")
...
let eval_leq arg1 arg2 =
  ...
  |Atom astr -> let a = int_of_string astr in
                ...
                |Cons _ -> raise (Error "add only works with int")
                |Atom bstr -> let b = int_of_string bstr in
                              if a < b then (Atom "t") else (Atom "nil")
...
let all_builtins =
  [ (* Special forms *)
    ...
    "add_list", BuiltinFn1 eval_add_list;
